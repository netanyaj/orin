"use client";
import { TripInfo } from "@/app/trip/_components/ChatBox";
import { Calendar, Currency, Users, Wallet2 } from "lucide-react";
import {
  useMotionValueEvent,
  useScroll,
  useTransform,
...
export const Timeline = ({ data, tripData }: { data: TimelineEntry[], tripData: TripInfo | null }) => {
    ...
    >
      ...
      <div ref={ref} className="relative max-w-7xl mx-auto pb-20">
        {data.map((item, index) => (
          <div
            key={index}
            className="flex justify-start pt-10 md:pt-10 md:gap-10 mb-10"
          >
            <div className="sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-full">
              <div className="h-10 absolute left-3 md:left-3 w-10 rounded-full bg-white dark:bg-black flex items-center justify-center">
                <div className="h-4 w-4 rounded-full bg-neutral-200 dark:bg-neutral-800 border border-neutral-300 dark:border-neutral-700 p-2" />
              </div>
              <h3 className="hidden md:block text-2xl md:pl-20 md:text-2xl font-bold text-neutral-500 dark:text-neutral-500 ">
                {item.title}
              </h3>
            </div>
